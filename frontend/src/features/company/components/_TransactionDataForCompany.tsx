...
export default function TransactionData({ tx }: TransactionDataProps) {
    ...
    const toggleCardExpansion = async (id: number) => {
        ...
        if (!expandedCards[id]) {
            setLoading(true);
            authCompanyAPI.getTransactionOrderItems(id)
            .then(response => {
                ...
                <VStack align="stretch" spaceY={3}>
                    ...
                    >
                        ...
                        >
                            ...
                            <Link href={`/company/transaction-detail/${tx.transaction_hash}`}>Detalles</Link>
                        </HStack>
                        <Text 
                            fontWeight="bold" 
                            textAlign="right"
                    ...
                    <HStack direction={'row'} justify={'space-between'}>
                        ...
                        {tx.status === 'confirmed' && (
                            ...
                            >
                                ...
                                {expandedCards[tx.id] ? 'Ocultar' : 'Resumen de la compra'}
                            </IconButton>                            
                        )}                    
                    </HStack>
                </VStack>
